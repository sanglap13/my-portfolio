...
export default function ExperienceHero({ title, subtitle, sequence }: { title: string; subtitle: string; sequence: SequenceConfig }) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [imagesLoaded, setImagesLoaded] = useState(false);
  const imagesRef = useRef<HTMLImageElement[]>([]);
  const FRAME_COUNT = sequence.frameCount || 192;
    ...
    <section className="relative h-screen w-full flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 text-center px-6 max-w-5xl mx-auto">
        ...
        <motion.p
          ...
          className="geist-mono text-sm tracking-[0.3em] uppercase text-theme-indigo/70 mb-8"
        >
      ...
      {/* Scroll indicator — same mouse-wheel pill as the home page */}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
